...
调用前设置环境变量（已为 Shaun 预配置）：
```
LOVART_ACCESS_KEY=ak_2324d3ef5ae927c4f23f694a1ac5893f
LOVART_SECRET_KEY=sk_1071c58e546ebe10e815e88161f4a002e048afe93ad60078005018fa41d2041c
```

脚本路径：`C:/Users/杨顺/.claude/skills/lovart-api/agent_skill.py`
默认输出目录：`C:/Users/杨顺/Documents/Obsidian Vault/Lovart/`

# ⚠️ 高岩品牌套件（GAOYAN BRAND KIT）

当用户请求中包含"高岩"、"餐观"、"GAOYAN"、"gaoyan"等关键词时，**必须**在 prompt 中注入以下品牌规范，不得省略：

```
Brand guidelines (MUST follow strictly):
- Primary color: GAOYAN Blue RGB(35,85,200) / #2355C8
- Accent color: GAOYAN Cyan RGB(0,240,190) / #00F0BE
- Supporting colors: White #FFFFFF, Black #000000
- Chinese headline font: 抖音美好体 (Douyin Meihao Ti)
- English headline font: GILL SANS MT BOLD
- Body font: 思源黑体 (Source Han Sans) / GILL SANS MT SEMIBOLD
- Slogan: 数智餐饮，共创味来
- Logo text: 高岩科技 / GAOYAN Technology
- Visual style: Professional, modern, tech-forward, data-driven
- Super graphic: Geometric shape in brand colors, can be cropped from edges (not rotated, not >50% cropped)
```

**触发词**：高岩、餐观、GAOYAN、gaoyan、高岩科技、高岩风味、高岩洞察

**注入方式**：将品牌规范追加到用户 prompt 之后，例如：
- 用户说："生成一张高岩科技的封面图"
- 实际 prompt：`生成一张高岩科技的封面图\n\n[Brand guidelines (MUST follow strictly): Primary color: GAOYAN Blue RGB(35,85,200)...]`
