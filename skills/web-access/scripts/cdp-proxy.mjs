...
async function getWebSocketUrl(port, wsPath) {
  ...
  // 从 Chrome HTTP API 获取带 UUID 的完整 WebSocket URL
  try {
    const res = await fetch(`http://127.0.0.1:${port}/json/version`, { signal: AbortSignal.timeout(3000) });
    const data = JSON.parse(await res.text());
    if (data.webSocketDebuggerUrl) {
      console.log(`[CDP Proxy] 从 /json/version 获取 wsUrl: ${data.webSocketDebuggerUrl}`);
      return data.webSocketDebuggerUrl;
    }
  } catch (e) {
    console.error('[CDP Proxy] 获取 wsUrl 失败:', e.message);
  }
