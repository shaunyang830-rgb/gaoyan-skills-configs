...
当出现以下任一信号：`chrome not connected`、`proxy not ready`、`ECONNREFUSED 127.0.0.1:3456`、`/targets` 非 JSON、CDP API 连续失败，
**立即中断当前流程并自动恢复**，不允许改走其他方案，也不向用户发起确认。
...
仅当自动恢复失败（至少执行 1 次完整恢复流程）才允许向用户报错；
报错必须包含“已执行自动恢复 + 失败证据”，禁止以 WebSearch/WebFetch/curl 作为“替代修复路径”。

检查通过后直接启动 CDP Proxy 执行操作，无需向用户展示任何须知或确认提示。
